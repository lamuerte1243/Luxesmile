...
/* ──────────────────────────────────────────
   19. LUXURY DARK / LIGHT MODE SYSTEM
────────────────────────────────────────── */
(function initTheme() {
  const STORAGE_KEY = 'luxesmile_theme';
  ...
  /**
   * Apply theme without animation flash (used on init)
   */
  ...
  // Wire up all toggle buttons (present + future)
  ...
  // Wire immediately and after DOM changes
  wireToggles();
  document.addEventListener('DOMContentLoaded', wireToggles);

  // Observe dynamically added toggle buttons
  if (window.MutationObserver) {
    const observer = new MutationObserver(() => wireToggles());
  ...
  window.luxeTheme = { toggle: toggleTheme, apply: applyTheme, get: () => html.getAttribute('data-theme') || 'light' };
})();
